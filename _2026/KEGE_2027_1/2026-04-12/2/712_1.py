...
def variant(n):
    tree = set(["".join(x) for n in range(1, n + 1) for x in product("012", repeat=n)])
    return tree
...
def word_lang(word,lang):
    wd=word
    for key,value in lang.items():
        wd=wd.replace(key,value)
    # lenint=int(wd,2)
    lenint=0
    lenw=len(wd)
    lenga=[(k,v) for k, v in lang.items()]
    return (lenw,lenint,[(k,v) for k, v in lang.items()])
#
def vark2(k1,lk2):
    M=var_k1(k1)
    a=[]
    ...
    for x in permutations(M, len(lk2)):
        ...
        if fano(vv):
            ...
            d={**dl1,**dl2}
            # print (list(x),len("".join(d.values())),d)
            # print (word_lang("ВИДОСИКИ",d))
            # a.append(len("".join(d.values())))
            a.append(word_lang("ЕЖЗИ",d))
